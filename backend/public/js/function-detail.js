document.addEventListener("DOMContentLoaded", () => {
  const fnId = document.body.dataset.functionId || "<%= fn.id %>";
  ...
  document.addEventListener('click', async (e) => {
    ...
    try {
      ...
      if (Array.isArray(comms) && comms.length > 0) {
        commsHTML += "<ul>" + comms.map(c => `<li>${c.entry_type}: ${c.subject || c.body}</li>`).join("") + "</ul>";
      } else {
  ...
  document.addEventListener('click', async (e) => {
    ...
    if (e.target.classList.contains('remove-btn')) {
      ...
      showToast('Contact removed'); location.reload();
    }

    if (e.target.classList.contains('delete-btn')) {
      if (!confirm('Delete this contact permanently?')) return;
      await fetch(`/functions/contacts/${id}/delete`, { method: 'DELETE' });
      showToast('Contact deleted'); location.reload();
    }
    ...
    if (e.target.classList.contains('primary-btn')) {
      ...
      showToast('⭐ Primary contact set'); location.reload();
    }
  });
  ...
  tabNew.addEventListener('click', () => {
    ...
    newForm.classList.remove('hidden'); existingSection.classList.add('hidden');
  });

  tabExisting.addEventListener('click', async () => {
    tabExisting.classList.add('active'); tabNew.classList.remove('active');
    newForm.classList.add('hidden'); existingSection.classList.remove('hidden');
    await loadContacts();
  });
  ...
  async function loadContacts() {
    ...
    selectDropdown.innerHTML = contacts.map(c => `
