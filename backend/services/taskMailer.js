 ...
 * @returns {Promise<{subject: string, body_html: string, body_text: string}>}
 */
async function sendTaskAssignmentEmail(accessToken, task, assignedUser, assignedBy) {
  ...
  const bodySections = [
    ...
    <strong>Due:</strong> ${
      task.due_at ? new Date(task.due_at).toLocaleDateString("en-NZ") : "No due date"
    }</p>`,
    `<p><a href="${dashboardLink}">View task in dashboard</a></p>`,
    `<hr>
